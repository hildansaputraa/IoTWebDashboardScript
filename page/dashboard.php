...
<body class="hold-transition sidebar-mini">
  ...
  <script>
    ...
    // Fungsi untuk check logika otomatis
    function checkAutoControl(solenoidNum) {
        if (solenoidState[solenoidNum].mode !== 'auto') return;

        // Ambil pengaturan dari input (atau simpan di state)
        const thresholds = {};
        let allValid = true;

        for (let node = 1; node <= 4; node++) {
            const aInput = document.getElementById(`threshold-node${node}-waterlvA-solenoid${solenoidNum}`);
            const bInput = document.getElementById(`threshold-node${  node}-waterlvB-solenoid${solenoidNum}`);
            
            const threshA = parseFloat(aInput?.value);
            const threshB = parseFloat(bInput?.value);
            ...
            thresholds[node] = { A: threshA, B: threshB };
        }

        if (!allValid) return;

        const action = document.getElementById(`action-solenoid${solenoidNum}`).value;
        const targetState = action === 'on' ? 1 : 0;

        // Cek SEMUA node memenuhi A dan B
        let shouldActivate = true;
        for (let node = 1; node <= 4; node++) {
            const currentA = sensorData[node].waterlvA;
            const currentB = sensorData[node].waterlvB;
            const threshA = thresholds[node].A;
            const threshB = thresholds[node].B;

            if (currentA < threshA || currentB < threshB) {
